# Prevent_Reuploading_of_same_File_1 — CodeIgniter 4.7.4

This is the **Prevent_Reuploading_of_same_File_1** exercise for the **CodeIgniter 4.7.4** framework.

## About the Exercise

The **Prevent_Reuploading_of_same_File_1** exercise demonstrates how to prevent reuploading of a single file to the `writable/uploads` folder in CodeIgniter 4.

## How This Works
1. The uploaded file is validated using the `$this->validate()` method along with validation rules for file size and file extensions.
2. Then the File Upload Errors are checked to see if the file is uploaded successfully.
3. Then a 40 characters long unique hash is generated based on uploaded file's content using sha1_file() function.
4. Then it is checked whether the file whose name is (unique hash.uploaded file extension) exists in writable/uploads folder.
5. If not exists, then move the uploaded file to that location. Otherwise, show error to the user.
...
Follow these steps to successfully execute the **Prevent_Reuploading_of_same_File_1** exercise.
...
Navigate to the `Prevent_Reuploading_of_same_File_1` exercise folder.
...
app.baseURL = 'http://localhost/the-location-to-your-Prevent_Reuploading_of_same_File_1-project-folder/public/'
```

Replace:

```text
the-location-to-your-Prevent_Reuploading_of_same_File_1-project-folder
```

with the actual location/path of your `Prevent_Reuploading_of_same_File_1` project folder.

For example:

```dotenv
app.baseURL = 'http://localhost/Prevent_Reuploading_of_same_File_1/public/'
```
...
```dotenv
CI_ENVIRONMENT = development
app.baseURL = 'http://localhost/Prevent_Reuploading_of_same_File/public/'
```
...
The **Main Page** of the **Prevent_Reuploading_of_same_File_1** exercise should now be displayed in your web browser.
...
This exercise demonstrates how to prevent reuploading of single-file and validation in CodeIgniter v4.7.4 using:

* `$this->validate()`
* File extension validation
* File size validation
* Single-file uploading
* Checking uploaded file uniqueness using sha1_file() function.
