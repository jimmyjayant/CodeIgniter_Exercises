# Register_Login Exercise – CodeIgniter 4.7.4

This README provides the steps required to successfully execute the **Register_Login exercise** using the **CodeIgniter 4.7.4** framework on Windows 11 with XAMPP.
...
### 2. Open the Register_Login Project Folder

Open a Terminal or Command Prompt and navigate to the **Register_Login** project folder.

For example:

```bash
cd path/to/Register_Login
```
...
Inside the **Register_Login** project folder, locate the copy of the `env` file.
...
app.baseURL = 'http://localhost/the location to your Register_Login project folder/public/'
```

Replace:

```text
the location to your Register_Login project folder
```

with the actual location of your Register_Login project.

For example:

```dotenv
app.baseURL = 'http://localhost/Register_Login/public/'
```
...
database.default.database = ci4_reg_user
...
```bash
php spark db:create ci4_reg_user
```

Here, `ci4_reg_user` is the name of the MySQL database.

> **Important:** Make sure that a database named `ci4_reg_user` does **not** already exist before running this command.

After successful execution, a new database named **ci4_reg_user** should be created.
...
You should see the `ci4_reg_user` database listed in phpMyAdmin.

### 9. Create the Migration

Make sure the Terminal is still open in the **Register_Login** project folder.
...
### 10. Open the Register_Login Application
...
```text
http://localhost/Register_Login/public/
```

Paste the URL into your web browser and press **Enter**.

The **main page of the Register_Login exercise** should now be displayed.
...
* The `ci4_reg_user` database exists in phpMyAdmin.
* The `.env` file is located in the root of the Register_Login project.
...
* The Terminal is opened in the correct **Register_Login** project directory.

After completing all the steps above, the **Register_Login CodeIgniter 4.7.4 exercise** should be ready to run.
